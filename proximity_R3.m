...
for j=1:size(FBS,2) %set power and status of each FBS
    fbs = FBS{j};
...
[G,L]=measure_channel(FBS,BS,selectedMUE,NumRealization);
selectedMUE.SINR=SINR_MUE_4(G,L,FBS,BS,-120);
...
for episode = 1:Iterations
    ...
    SINR_FUE_Vec = SINR_FUE_2(G,L,FBS, BS, -120);
    selectedMUE = selectedMUE.setCapacity(log2(1+SINR_MUE_4(G,L,FBS, BS, -120)));
